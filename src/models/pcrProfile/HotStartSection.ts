...
export default class HotStartSection extends PcrSection {
  step1: PcrStep;
  step2: PcrStep;
  numberOfCycle = 10;
  constructor(label: string) {
    super(label, PcrSectionType.HotStart);
  ...
  public static emotyInstance(label: string) {
    const result = new HotStartSection(label);
  ...
  clone = (): HotStartSection => {
    const json = JSON.stringify(this);
    const obj = JSON.parse(json);
    const result = Object.assign(new HotStartSection(obj.label), obj);
    result.updateViewID();
    return result;
  }

  tooltipString = () => {
    return this.enabled ? `Hot Start = ${this.step1.temp} - ${this.step2.temp} (${this.numberOfCycle}x)` : 'Hot Start = -';
  }
